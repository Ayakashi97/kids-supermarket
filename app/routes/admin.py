...
def extract_hostname(url_str: str) -> str:
    """Extract lowercase hostname from a URL or domain string."""
...
def settings():
    ...
    if request.method == "POST":
        # Card 1: Supermarkt-Einstellungen
        if "shop_name" in request.form:
            set_setting("shop_name", request.form.get("shop_name", Config.SHOP_NAME))
        if "admin_pin" in request.form:
            set_setting("admin_pin", request.form.get("admin_pin", Config.ADMIN_PIN))
        if "nfc_mode" in request.form:
            set_setting("nfc_mode", request.form.get("nfc_mode", "web_nfc"))
        if "scanner_mode" in request.form:
            set_setting("scanner_mode", request.form.get("scanner_mode", "disabled"))
        if "pin_mode" in request.form:
            set_setting("pin_mode", request.form.get("pin_mode", "disabled"))
        if "screen_timeout" in request.form:
            set_setting("screen_timeout", request.form.get("screen_timeout", "30"))
        if "show_nfc_toast" in request.form:
            set_setting("show_nfc_toast", request.form.get("show_nfc_toast", "true"))

        # Card 3: Bon-Drucker Hardware-Einstellungen
        if "printer_enabled" in request.form:
            set_setting("printer_enabled", request.form.get("printer_enabled", "false"))
        if "print_mode" in request.form:
            set_setting("print_mode", request.form.get("print_mode", "ask_cashier"))
        if "max_prints_per_hour" in request.form:
            set_setting("max_prints_per_hour", request.form.get("max_prints_per_hour", "20"))
        if "printer_device" in request.form:
            set_setting("printer_device", request.form.get("printer_device", Config.PRINTER_DEVICE))
        if "paper_width" in request.form:
            set_setting("paper_width", request.form.get("paper_width", "58mm"))

        # Card 4: Visueller Kassenbon Template-Builder
        if "receipt_layout_json" in request.form:
        ...
        # Card 5: Entwickler-Einstellungen
        ...
        # Card 2: HTTPS & SSL-Zertifikat (now includes base_url)
        if "base_url" in request.form:
            user_base_url = request.form.get("base_url", "").strip()
            set_setting("base_url", user_base_url)
        ...
        if cert_uploaded or key_uploaded:
            ...
            if cert_uploaded:
                ...
                flash("HTTPS wurde erfolgreich aktiviert. Bitte den Container neu starten, damit die Änderungen wirksam werden!", "popup")

        if "ssl_enabled" in request.form:
            ...
            if ssl_enabled == "true":
                ...
                if not cert_path_setting or not key_path_setting or not os.path.exists(cert_path_setting) or not os.path.exists(key_path_setting):
                    ...
                    try:
                        ...
                        flash(f"Fehler beim Laden des Zertifikat-Paares ({ssl_err})! HTTPS bleibt deaktiviert.", "danger")
                        ssl_enabled = "false"

            set_setting("ssl_enabled", ssl_enabled)

        # Auto-fill base_url if empty and cert exists
        ...
        if cert_info and ("base_url" in request.form or "ssl_enabled" in request.form or cert_uploaded):
            if not current_base_url and cert_info.get("common_name"):
                scheme = "https" if get_setting("ssl_enabled", "false") == "true" else "http"
                auto_url = f"{scheme}://{cert_info['common_name']}"
                set_setting("base_url", auto_url)

        # Only flash generic success if no other card flashed a message
        if not session.get("_flashes"):
            flash("Einstellungen erfolgreich gespeichert! ⚙️", "success")
...
def delete_cert():
    ...
    try:
        ...
        # Revert base_url from https:// to http://
        base_url = get_setting("base_url", "")
        if base_url:
        ...
        flash("Zertifikate gelöscht & auf HTTP (unverschlüsselt) zurückgesetzt! 🔓", "success")
        flash("Das SSL-Zertifikat wurde gelöscht und das System wurde auf HTTP zurückgesetzt. Bitte den Container neu starten, damit der Server auf Port 80 umschaltet!", "popup")
    except Exception as e:
        flash(f"Fehler beim Löschen des Zertifikats: {e}", "danger")

    # Redirect explicitly to http:// scheme so browser leaves https://
    host = request.host.split(":")[0]
    http_port = Config.HTTP_PORT
    if http_port and str(http_port) != "80":
        redirect_url = f"http://{host}:{http_port}{url_for('admin.settings')}"
    else:
        redirect_url = f"http://{host}{url_for('admin.settings')}"
