...
def find_card_by_uid(uid: str) -> Card:
    """Find card by NFC UID using indexed SQL query with normalized fallback."""
    if not uid:
        return None
    # 1. Exact match (indexed, fast)
    card = Card.query.filter_by(nfc_uid=uid).first()
    if card:
        return card

    # 2. Normalized match — only iterate if exact match fails (rare edge case)
    # Handles UIDs with/without colons, dashes, spaces or different casing
    clean_uid = uid.replace(":", "").replace("-", "").replace(" ", "").strip().lower()
    if not clean_uid:
        return None

    # Use SQL LIKE for prefix, then normalize in Python (only reads cards table once)
    candidates = Card.query.filter(Card.nfc_uid.isnot(None)).all()
    for c in candidates:
...
def find_product_by_nfc_uid(uid: str):
    """Find an active product by its NFC tag UID using indexed SQL query with normalized fallback."""
    from app.models import Product
    if not uid:
        return None
    # 1. Exact match (indexed, fast)
    product = Product.query.filter_by(nfc_uid=uid, is_active=True).first()
    if product:
        return product
    ...
    if not clean_uid:
        return None
    for p in Product.query.filter_by(is_active=True).filter(Product.nfc_uid.isnot(None)).all():
