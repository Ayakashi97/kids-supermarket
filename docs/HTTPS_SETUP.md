...
### 🔥 Amazon Fire HD 10 / Fire OS (für Tablet als Kasse oder Terminal)
1. **Datei übertragen**: Lade `supermarket-ca.crt` auf das Fire HD 10 herunter (z. B. über den Silk-Browser via Server-Link, per USB-Kabel, E-Mail oder Cloud).
2. **Einstellungen öffnen**: Öffne die App **Einstellungen** (Zahnrad-Symbol).
3. **Zertifikate-Menü aufrufen**:
   - Tippe auf **Sicherheit & Datenschutz** (Security & Privacy).
   - Tippe auf **Anmeldedaten-Speicher** (Credential Storage) oder **Erweitert** ➡️ **Von Speicher / SD-Karte installieren** (Install from storage).
4. **Zertifikat importieren**:
   - Navigiere zum Ordner **Download** und wähle `supermarket-ca.crt` aus.
   - Gib deine Bildschirmsperre-PIN/Muster ein, falls danach gefragt wird.
   - Vergib den Zertifikatsnamen (z. B. `Kinder-Supermarkt CA`) und wähle bei Verwendung **VPN und Apps** oder **WLAN / CA-Zertifikat**.
   - Bestätige mit **OK** / **Installieren**.
5. **Browser neu starten**: Schließe Silk oder Chrome komplett und starte die App neu. Beim Aufruf von `https://supermarket.local` gilt die Verbindung nun als sicher 🔒.
